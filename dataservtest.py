...
def action_prompt(entity_type, entity):
    # Ask user to choose an action for the found enemy
    print(f"Choose an action for the {entity_type} '{entity}':")
    ...
    try:
        choice = int(input(f"Enter the number of the action you want to take (1-4): "))
        if 1 <= choice <= 4:
            return actions[choice - 1]
        else:
            print("Invalid choice, defaulting to 'Investigate'.")
            return "Investigate"
    except ValueError:
        print("Invalid input, defaulting to 'Investigate'.")
        return "Investigate"
